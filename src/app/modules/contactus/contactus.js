...
var contactusModule = angular.module('hod.contactus', [])
...
contactusModule.controller('ContactUsCtrl',['$scope', function ($scope) {
  ...
  $scope.contactusSubmit = function (valid) {
    if (valid) {
      ...
      body += '\n\n## Affected users ##\n' + data.users
      body += '\n\n## Comments ##\n' + data.other
      window.location = "mailto:provingthingsips@homeoffice.gsi.gov.uk?body=" + safe(body) + "&subject=" + safe(data.issueType.toUpperCase() + ' PTTG-IPS')
